...
void ReController::reset(){tracks_.clear();selectedTrack_.clear();trackEvents_.clear();session_.clear();sessions_.clear();result_.clear();lastError_.clear();emit changed();}
...
bool ReController::trackObject(const QString& name,const QString& address,const QString& pointerPath,int size,bool persist){json args{{"name",name.toStdString()},{"size",size},{"persist",persist}};if(!pointerPath.trimmed().isEmpty())args["pointer_path"]=pointerPath.trimmed().toStdString();else args["address"]=AddressValue(address);json r;if(!call("re_track_object",args,r,true))return false;setResult(r);refresh();return true;}
