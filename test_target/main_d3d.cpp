...
//
// Pick the backend via -DTEST_RENDERER=9 or =11 at compile time; the CMake
// build produces test_target_d3d9_{x86,x64}.exe and test_target_d3d11_{...}.
...
static HWND MakeWindow(const char* title) {
    WNDCLASSA c{}; c.lpfnWndProc = WndProc; c.hInstance = GetModuleHandleA(nullptr);
    c.lpszClassName = "cortex_test_d3d"; c.hCursor = LoadCursor(nullptr, IDC_ARROW);
    RegisterClassA(&c);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    p.BackBufferFormat = D3DFMT_X8R8G8B8; p.hDeviceWindow = wnd;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT; sd.OutputWindow = wnd;
