...
std::string ConfiguredDebuggerBackend(const std::string& runtimeDirectory) {
    const json settings = DesktopSettings(runtimeDirectory);
    return LowerAscii(settings.value("debuggerBackend", std::string("windows"))) == "veh"
        ? "veh" : "windows";
}

std::string ConfiguredCrashDirectory(const std::string& runtimeDirectory) {
    const json settings = DesktopSettings(runtimeDirectory);
    return settings.value("diagnosticsCrashDirectory", std::string());
}
...
if(name=="cortex_crash_report"){std::string configured=ConfiguredCrashDirectory(s->runtimeDirectory);std::string e;auto bundle=cortex::services::CrashReportService::Latest(cortex::services::CrashReportService::DefaultRoots(s->runtimeDirectory,r->target.architecture,configured),r->target.processId,&e);json out={{"ok",e.empty()},{"found",bundle.found},{"target",TargetJson(r->target,true)},{"recent_events",EventsJson(s,0,64,r)}};if(bundle.found){out["directory"]=bundle.directory.u8string();out["report"]=bundle.report;out["symbolized"]=bundle.symbolized;out["hooks"]=bundle.hooks;out["breadcrumbs"]=bundle.breadcrumbs;}if(!e.empty())out["error"]=e;response=LocalToolResponse(MessageId(m),out,!e.empty());return true;}
