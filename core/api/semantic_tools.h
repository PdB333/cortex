...
inline json Catalog() {
    ...
    tools.push_back(Tool("record_interaction_window", "Plan a bounded observation window combining input, timeline, watches, screenshots, and session export.", {"input_record_start","input_record_stop","timeline_start","timeline_stop","watch_events","screenshot","session_export"}));
    ...
    tools.push_back(Tool("discover_event_correlations", "Plan repeated event observations and rank temporally correlated changes.", {"timeline_start","timeline_mark","watch_add","watch_events","snapshot_create","snapshot_diff"}));
    ...
    tools.push_back(Tool("trace_execution_from_event", "Plan a bounded execution trace linked to an input, prompt, breakpoint, or timeline marker.", {"trace_start","trace_events","trace_coverage","trace_callgraph","input_sequence","prompt_timed_test"}));
...
inline json PlanFor(const std::string& wanted, const json& arguments) {
    for (const auto& tool : Catalog()) {
        ...
        json result = {
            ...
            {"objective", arguments.value("objective", std::string())},
    ...
    return {{"status", "failed"}, {"error", "unknown_semantic_tool"}};
}
