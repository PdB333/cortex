	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				if ((D3D10CreateDeviceAndSwapChain = (void*)::GetProcAddress(libD3D10, "D3D10CreateDeviceAndSwapChain")) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
		...
		else if (_renderType != RenderType::Auto)
		{
			if (_renderType == RenderType::OpenGL)
			{
				...
				const char* const methodsNames[] = {
					...
					// Not part of upstream kiero: wglSwapBuffers is the actual
					// frame-present entry point for OpenGL, needed to drive an
					// overlay, but kiero's stock table only carries legacy
					// GL1.1 exports. Appended at the end so every existing
					// index in METHODSTABLE.txt stays valid; its own index is
					// simply the array's previous size (336).
					"wglSwapBuffers"
				};
