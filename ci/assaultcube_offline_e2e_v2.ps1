...
$report = [ordered]@{
    schema = "cortex.assaultcube.offline-e2e.v2"
    status = "running"
...
try {
    ...
    foreach ($required in @(
        $cortexExe,
        $serverExe,
        (Join-Path $CortexDir "runtime\x86\cortex_core.dll"),
        (Join-Path $CortexDir "runtime\x86\cortex_runtime_helper.exe")
    )) {
    ...
    $init = Send-McpRequest "initialize" @{
        ...
        capabilities = @{}
        clientInfo = @{ name = "assaultcube-offline-ci-v2"; version = "2" }
    }
    ...
    $permissionDenied = ($permissionProbe.PSObject.Properties.Name -contains "result") -and
        ($permissionProbe.result.PSObject.Properties.Name -contains "isError") -and $permissionProbe.result.isError -and
        ([string]$permissionProbe.result.structuredContent.error -eq "mutation_permission_required")
    ...
    if ([int](Get-Structured $targetsAfterDetach).count -ne 0) { throw "cortex_targets still reports AssaultCube after detach" }
    ...
    if ([int64](Get-RouteResult $statusAfterReattach).pid -ne [int64]$server.Id) { throw "status after re-attach reports the wrong PID" }
